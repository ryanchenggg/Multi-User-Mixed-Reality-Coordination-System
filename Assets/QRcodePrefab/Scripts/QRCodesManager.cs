...
namespace QRTracking
{
    ...
    public class QRCodesManager : Singleton<QRCodesManager>
    {
        ...
        //Avoid to  using past qrcode stored in OS
        private DateTimeOffset watcherStart;
        ...
        async protected virtual void Start()
        {
            ...
            IsSupported = QRCodeWatcher.IsSupported(); //Gets whether the QRCodeWatcher is supported on this device.
            capabilityTask = QRCodeWatcher.RequestAccessAsync(); //Requests access to the QRCodeWatcher.
        ...
        private void SetupQRTracking()
        {
            ...
            try
            {
                qrTracker = new QRCodeWatcher(); // get reference to QRCodeWatcher
                IsTrackerRunning = false; // it's not running yet
                ...
                qrTracker.EnumerationCompleted += QRCodeWatcher_EnumerationCompleted; // Event representing the completion of the enumeration of QR codes.

                //Initialize watcherStart to current time;
            ...
            if (AutoStartQRTracking) // Default is false
            {
                StartQRTracking();
            }
        }
        ...
        private void QRCodeWatcher_Added(object sender, QRCodeAddedEventArgs args)
        {
            ...
            if (args.Code.LastDetectedTime > watcherStart)
            {
                lock (qrCodesList)
                {
                    qrCodesList[args.Code.Id] = args.Code;
